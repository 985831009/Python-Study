...
def runs(df_a, df_b):
    ...
    # 一对多匹配
        ...
        for more_x in range(2, 6):
            ...
            #     print(f"{more_x}对{more_x}匹配：a表第{match[0]}行与b表第{match[1]}行匹配")
            return df_a2, df_b2
...
def process_rows_parallel(df_a, df_b):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as executor:
        results = [
            executor.submit(runs, group_a, group_b)
            for date, group_a in grouped_a
            for b_date, group_b in grouped_b
            if date == b_date
        ]

        results = [r.result() for r in concurrent.futures.as_completed(results)]
...
def Run_Files():
    try:
        ...
        read_df = process_rows_parallel(df_a, df_b)

        # Save the results to the output file
        read_df.to_excel(r"D:\RPADATA\GBS\P33_BankRecon_CKCN\Bot01\output/result.xlsx", index=False)
